...
def compute_kuramoto_gradients(phases, weights, biases, bias_phases):
    ...
    gradient_weights_forward = -phases[:, None]*phases[None, :]

    # Compute gradient_biases_forward and gradient_bias_phases_forward
    gradient_biases_forward = -phases
    gradient_bias_phases_forward = 0
...
def determine_K_binary_distance(phases, outputn, label):
    return jnp.abs(jnp.sum(phases[outputn] - label))
...
def main_K_training_preamble(N, T, dt, batch_size, random_init_times, inputn, outputn, rng_key, map_features_and_labels):
    ...
    init_phases = jax.random.uniform(rng_key, shape=(N,), minval=-1, maxval=1)/2 + 2
    amp_rel, features, labels= map_features_and_labels(100, -30, 0.3, init_phases, outputn)
