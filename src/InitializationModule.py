...
from .WeightsModule import create_weight_update_mask, create_triangular_lattice_connections
...
def initialize_neurons(N, inputn, lattice_connections):
    neurons = jnp.arange(0, N, 1)
    if lattice_connections=='a':
        connections_neuronwise = jnp.array([
            [element for element in neurons] # previously added 'if element != neuron'
            for neuron in neurons
        ])
    elif lattice_connections=='t':
        connections_neuronwise = create_triangular_lattice_connections(N)
    return neurons, connections_neuronwise
...
def initialize_weights_and_SL_fields(N, inputn, connections_neuronwise, rng_key, weight_option, min_value, high_value):
    weights_real_matrix = create_symmetric_weights(N, 0., 1/N, inputn, rng_key, options=weight_option)
    weights_imaginary_matrix = create_symmetric_weights(N, 0., 1/N, inputn, rng_key, options=weight_option)
    weight_update_mask = create_weight_update_mask(N, inputn, connections_neuronwise)
...
def initialize_SL_states_and_features(feature_multiplier, feature_constant, label_multiplier, init_amplitudes, init_phases, uField, inputn, outputn, map_features_and_labels, rng_key):
    amplitude_relative, features, labels = map_features_and_labels(feature_multiplier, feature_constant, label_multiplier, init_amplitudes, outputn, rng_key)
    uField = uField.at[jnp.array(inputn)].set(features[0])
